...
import { ArrowRight, BadgeCheck, BriefcaseBusiness, Check, Clock3, ExternalLink, Factory, Flame, Network, ShieldCheck, ShoppingBag, Sparkles, Store, Users, UtensilsCrossed } from "lucide-react";
import { alternateRoute, Lang, PageKey, pages, routes, ui } from "./content";
import { LocatorPage } from "./locator-page";
import { associationMembers, associationMembersSource } from "./members";
...
const partnerGroups = [
  {
    title: { fr: "Partenaires professionnels de l’ANMB", de: "Berufspartner der ANMB" },
    note: { fr: "Organisations présentées comme partenaires sur le site actuel de l’association.", de: "Organisationen, die auf der aktuellen Verbandswebsite als Partner aufgeführt sind." },
    partners: [
      { mark: "UPSV", name: "Union professionnelle suisse de la viande · Carnasuisse", href: "https://www.carnasuisse.ch/fr/", text: { fr: "Organisation nationale de la branche carnée.", de: "Nationale Organisation der Fleischbranche." } },
      { mark: "REN", name: "Réseau d’entreprises formatrices neuchâteloises", href: "https://reneuchatel.ch/", text: { fr: "Réseau cantonal consacré à la formation professionnelle.", de: "Kantonales Netzwerk für die Berufsbildung." } },
      { mark: "CNCI", name: "Chambre neuchâteloise du commerce et de l’industrie", href: "https://cnci.ch/", text: { fr: "Secrétariat patronal et relais économique cantonal.", de: "Arbeitgebersekretariat und kantonales Wirtschaftsnetzwerk." } },
    ],
  },
  {
    ...
    partners: [
      { mark: "IGP", name: "Association suisse des AOP-IGP", href: "https://www.aop-igp.ch/fr/saucisson-neuchatelois-igp", text: { fr: "Fiche officielle du Saucisson neuchâtelois IGP.", de: "Offizielle Produktseite des Neuenburger Saucisson IGP." }, officialLogo: true },
      { mark: "OIC", name: "Organisme intercantonal de certification", href: "https://www.oic-izs.ch/Prestations/AOP-IGP", text: { fr: "Organisme chargé du contrôle et de la certification.", de: "Stelle für Kontrolle und Zertifizierung." } },
      { mark: "OFAG", name: "Office fédéral de l’agriculture", href: "https://www.blw.admin.ch/fr/aop-igp-appellations-dorigine-et-indications-geographiques", text: { fr: "Registre fédéral des appellations AOP et IGP.", de: "Bundesregister der AOP- und IGP-Bezeichnungen." } },
    ],
  },
  {
    ...
    partners: [
      { mark: "ST", name: "Suisse Terroir", href: "https://www.suisseterroir.ch/suggestions/saucisson-neuchatelois-igp", text: { fr: "Présentation nationale du produit et de son aire d’origine.", de: "Nationale Präsentation des Produkts und seines Herkunftsgebiets." } },
      { mark: "PCS", name: "Patrimoine culinaire suisse", href: "https://www.patrimoineculinaire.ch/Produit/Saucisson-neuchatelois-IGP-saucisse-neuchateloise-IGP/29", text: { fr: "Histoire, usages, fabrication et tradition de la torrée.", de: "Geschichte, Verwendung, Herstellung und Torrée-Tradition." } },
      { mark: "NVT", name: "Neuchâtel Vins et Terroir", href: "https://neuchatel-vins-terroir.ch/", text: { fr: "Promotion des produits et vins du terroir neuchâtelois.", de: "Förderung der Neuenburger Weine und Regionalprodukte." } },
    ],
  },
  {
    ...
    partners: [
      { mark: "PV", name: "Proviande · Viande Suisse", href: "https://www.proviande.ch/fr", text: { fr: "Interprofession suisse de la filière viande.", de: "Schweizer Branchenorganisation der Fleischwirtschaft." } },
      { mark: "ST", name: "Suisse Tourisme · Neuchâtel", href: "https://www.myswitzerland.com/fr-ch/destinations/neuchatel/", text: { fr: "Page officielle de la destination Neuchâtel.", de: "Offizielle Seite der Destination Neuenburg." } },
      { mark: "J3L", name: "Jura & Trois-Lacs · Pays de Neuchâtel", href: "https://www.j3l.ch/fr/", text: { fr: "Plateforme touristique de la région et du Pays de Neuchâtel.", de: "Tourismusplattform der Region und des Pays de Neuchâtel." } },
    ],
  },
];
...
function AssociationSubnav({ lang, current }: { lang: Lang; current: PageKey }) {
  const links: [PageKey, string][] = lang === "fr"
    ? [["association", "L’association"], ["members", "Membres ANMB"], ["committee", "Comité ANMB"], ["partners", "Réseau & partenaires"]]
    : [["association", "Der Verband"], ["members", "ANMB-Mitglieder"], ["committee", "ANMB-Vorstand"], ["partners", "Netzwerk & Partner"]];
...
export function HomePage({ lang }: { lang: Lang }) {
  ...
  const pathways = lang === "fr" ? [
    ...
    { icon: ShoppingBag, tag: "Acheter", title: "Une adresse fiable près de vous", text: "Fabricants recensés et points de vente, séparés des membres ANMB.", key: "locator" as PageKey },
  ] : [
    ...
    { icon: ShoppingBag, tag: "Kaufen", title: "Eine verlässliche Adresse", text: "Erfasste Hersteller und Verkaufsstellen, getrennt von ANMB-Mitgliedern.", key: "locator" as PageKey },
  ];
  ...
  return <>
    ...
    <main id="contenu">
      ...
      <section className="locator-preview section-pad"><div className="locator-copy"><p className="eyebrow">{lang === "fr" ? "Circuit de confiance" : "Vertrauenswürdige Adressen"}</p><h2>{t.locatorTitle}</h2><p>{t.locatorText}</p><div className="locator-preview-stats"><span><strong>9</strong>{lang === "fr" ? "fabricants recensés" : "erfasste Hersteller"}</span><span><strong>14</strong>{lang === "fr" ? "adresses de vente" : "Verkaufsadressen"}</span></div><ButtonLink href={routes[lang].locator}>{lang === "fr" ? "Ouvrir la carte à jour" : "Aktuelle Karte öffnen"}</ButtonLink></div><figure className="locator-photo"><Image src={assetPath("/aop-paysage.webp")} alt={lang === "fr" ? "Paysage du canton de Neuchâtel" : "Landschaft des Kantons Neuenburg"} fill sizes="(max-width: 760px) 100vw, 55vw" /><figcaption>{lang === "fr" ? "Photo : Association suisse des AOP-IGP" : "Foto: Schweizerische Vereinigung der AOP-IGP"}</figcaption></figure></section>
    ...
    <main id="contenu">
      ...
      {pageKey === "association" && <>
        ...
        <section className="association-paths section-pad">
          <Link href={routes[lang].members}><BriefcaseBusiness size={28} /><span>{lang === "fr" ? "Réseau professionnel" : "Berufsnetzwerk"}</span><h2>{lang === "fr" ? "Voir les membres ANMB" : "ANMB-Mitglieder ansehen"}</h2><ArrowRight size={20} /></Link>
      ...
      {pageKey === "committee" && <section className="committee-section section-pad">
        ...
        <aside className="editorial-note"><Clock3 size={23} /><p>{page.note} <a href="https://www.boucheries-neuchatel.ch/Presentation/Le-comite-ANMB" target="_blank" rel="noreferrer">{lang === "fr" ? "Consulter la source" : "Quelle öffnen"}<ExternalLink size={14} /></a></p></aside>
        <section className="igp-governance"><div className="igp-governance-heading"><p className="eyebrow">{lang === "fr" ? "Responsabilités des deux IGP" : "Verantwortung für die zwei IGP"}</p><h2>{lang === "fr" ? "Une gouvernance lisible, sans inventer une seconde association." : "Klare Zuständigkeiten ohne einen zweiten Verband zu erfinden."}</h2><p>{lang === "fr" ? "Les sources officielles désignent l’ANMB comme contact des deux produits. Elles ne publient pas de fonction intitulée « président du saucisson »." : "Die offiziellen Quellen nennen die ANMB als Kontaktorganisation beider Produkte. Eine Funktion « Präsident des Saucisson » wird nicht publiziert."}</p></div><div className="responsibility-grid">
          ...
          <article className="responsibility-person"><BadgeCheck size={25} /><p>{lang === "fr" ? "Référent ANMB des deux IGP" : "ANMB-Ansprechpartner für beide IGP"}</p><h3>Arthur Montandon</h3><span>{lang === "fr" ? "Intitulé descriptif proposé pour le site; la dénomination statutaire du mandat reste à confirmer par un procès-verbal ou une décision ANMB." : "Beschreibende Bezeichnung für die Website; die statutarische Amtsbezeichnung ist noch mit einem ANMB-Protokoll oder Beschluss zu bestätigen."}</span></article>
