...
function Brand() {
  return <span className="brand" aria-label="Saucisson neuchâtelois IGP"><Image className="brand-official-mark" src="/logo-igp-officiel.jpg" alt="" width={54} height={52} /><span className="brand-name">Saucisson<small>neuchâtelois · IGP</small></span></span>;
}
...
function Footer({ lang }: { lang: Lang }) {
  ...
  return <footer className="site-footer"><div className="footer-main">
    <div className="footer-identity"><Brand /><p>{lang === "fr" ? "Une spécialité protégée, un canton à découvrir." : "Eine geschützte Spezialität, ein Kanton zum Entdecken."}</p><div className="anmb-lockup"><Image src="/logo-anmb-boucherie.svg" alt="Boucherie neuchâteloise" width={76} height={76} /><div><strong>ANMB</strong><span>{lang === "fr" ? "Association neuchâteloise des maîtres-bouchers" : "Neuenburger Metzgermeister-Verband"}</span><Image className="anmb-signature" src="/logo-anmb-signature.svg" alt={lang === "fr" ? "Ma chair et tendre" : "Ma chair et tendre"} width={188} height={21} /></div></div></div>
  ...
  </div><div className="footer-bottom"><span>Prototype stratégique · Saucisson neuchâtelois IGP</span><span>{lang === "fr" ? "Logotypes officiels ANMB et AOP-IGP Suisse · Visuel de torrée original." : "Offizielle Logos von ANMB und AOP-IGP Schweiz · Eigenständiges Torrée-Motiv."}</span></div></footer>;
}
  ...
  return <>
    ...
    <main id="contenu">
      <section className="hero">
        ...
        <figure className="hero-image"><Image src="/torree-hero.webp" alt={lang === "fr" ? "Saucisson fumé tranché près des braises lors d’une torrée" : "Geräucherter Saucisson neben der Glut einer Torrée"} fill priority sizes="(max-width: 900px) 100vw, 55vw" /><figcaption>{t.imageCaption}</figcaption></figure>
      </section>
...
export function DetailPage({ lang, pageKey }: { lang: Lang; pageKey: PageKey }) {
  ...
  return <><Header lang={lang} current={pageKey} /><main id="contenu"><section className="detail-hero"><div><p className="eyebrow light">{page.eyebrow}</p><h1>{page.title}</h1><p>{page.intro}</p></div><div className="detail-orbit"><Image src="/logo-igp-officiel.jpg" alt={lang === "fr" ? "Logo officiel IGP" : "Offizielles IGP-Logo"} width={132} height={126} /></div></section><Facts items={page.facts} /><section className="detail-content section-pad">{page.sections.map((section, index) => <article className="detail-section" key={section.title}><div className="section-number">{String(index + 1).padStart(2, "0")}</div><div><p className="eyebrow">{section.kicker}</p><h2>{section.title}</h2><p>{section.text}</p>{section.bullets && <ul>{section.bullets.map((bullet) => <li key={bullet}><Check size={18} />{bullet}</li>)}</ul>}</div></article>)}{page.note && <aside className="editorial-note"><Clock3 size={23} /><p>{page.note}</p></aside>}</section><section className="next-step"><div><p className="eyebrow light">{lang === "fr" ? "À vous de jouer" : "Jetzt sind Sie dran"}</p><h2>{pageKey === "locator" ? (lang === "fr" ? "Une liste fiable se construit ensemble." : "Eine verlässliche Liste entsteht gemeinsam.") : (lang === "fr" ? "Du savoir au plaisir, il n’y a qu’un pas." : "Vom Wissen zum Genuss ist es nur ein Schritt.")}</h2></div><ButtonLink href={pageKey === "locator" ? routes[lang].association : routes[lang].locator}>{pageKey === "locator" ? "ANMB" : ui[lang].find}</ButtonLink></section></main><Footer lang={lang} /></>;
}
